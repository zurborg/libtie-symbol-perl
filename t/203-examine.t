...
is_deeply \@abc_symbols => [qw[ $scalar %hash &code @array def ]], 'symbols in abc';
is_deeply \@def_symbols => [qw[ $scalar %hash &code @array ]], 'symbols in abc::def';
...
is_deeply $ST_abc->tree => { def => {} }, 'tree';
is_deeply $ST_abc => {
    '$scalar' => \$abc::scalar,
    '@array' => \@abc::array,
    '%hash' => \%abc::hash,
